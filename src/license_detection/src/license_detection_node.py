...
# 导入自定义消息类型
from my_interfaces.msg import LicenseInfo
# 获取包的路径
import rospkg

# 获取当前包的路径
rospack = rospkg.RosPack()
package_share_directory = rospack.get_path('license_detection')
...
def get_plate_rec_landmark(img, xyxy, conf, landmarks, class_num, device, plate_rec_model, is_color=False): 
    ...
    for dan in danger:   #只要出现'危'或者'险'就是危险品车牌
        if dan in plate_number:
            plate_number='危险品'
    result_dict['license_no']=plate_number   #车牌号
...
def draw_result(orgimg, dict_list, is_color=False):   
    ...
    for result in dict_list:
        ...
        if length < 30:
            continue
        if not result['license_no']:
            continue
        green = (0, 255, 0) if result['license_no'] != '危险品' else (0, 0, 255)
        ...
        orgimg = cv2ImgAddText(orgimg, result['license_no'], int(x), int(y - 30), (0, 0, 0), 20)
        # orgimg = cv2ImgAddText(orgimg,result['license_no'],(int(x),int(y)),(0,0,0),20)
        result_str += result['license_no'] + " "
    return orgimg

class LicenseDetectionNode:
    def __init__(self):
        rospy.init_node('license_detection_node')
        
        # 获取参数
        detect_model_path = rospy.get_param('~detect_model_path', 'weights/plate_detect.pt')
        rec_model_path = rospy.get_param('~rec_model_path', 'weights/plate_rec_color.pth')
        self.is_color = rospy.get_param('~is_color', True)
        self.img_size = rospy.get_param('~img_size', 640)
        input_topic = rospy.get_param('~input_topic', '/image_raw')
        output_topic = rospy.get_param('~output_topic', '/license_detection_result')
        result_image_topic = rospy.get_param('~result_image_topic', '/license_detection_result_image')
        ...
        if not os.path.exists(detect_model_path):
            rospy.logerr(f'Detection model not found: {detect_model_path}')
            return
        if not os.path.exists(rec_model_path):
            rospy.logerr(f'Recognition model not found: {rec_model_path}')
            return
        ...
        rospy.loginfo('License detection node initialized')
        rospy.loginfo(f'Subscribing to: {input_topic}')
        rospy.loginfo(f'Publishing results to: {output_topic}')
        rospy.loginfo(f'Publishing result images to: {result_image_topic}')
    ...
    def image_callback(self, msg):
        try:
            ...
            if dict_list:
                ...
                for result in dict_list:
                    # 确保车牌号存在且不为空白字符
                    if result['license_no'] and result['license_no'].strip():
                        ...
                        result_msg.license_no = result['license_no'].strip()
                        result_msg.center_x = center_x
                        result_msg.center_y = center_y
                        
                        # 为当前这一个车牌发布独立的消息
                        self.result_pub.publish(result_msg)
                        rospy.loginfo(f"Published license: {result_msg.license_no} at ({center_x:.2f}, {center_y:.2f})")
            
        except Exception as e:
            rospy.logerr(f'Error processing image: {str(e)}')

def main():
    node = LicenseDetectionNode()
    ...
    try:
        ...
        rospy.loginfo("Shutting down license detection node")
